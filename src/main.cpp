...
unsigned int nStakeMinAge = 9 * 60 * 60;// 9 Hrs
...
const string strMessageMagic = "VestxCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
            ...
            if(nBestHeight <= 250)
            {
            nSubsidy = 1;  //no  pos reward until block 250
            }

            else if(nBestHeight <= 25000)
            {
            nSubsidy = nSubsidy * 30 ;  //1500% until block 25k
            }

            else if(nBestHeight <= 35000)
            {
            nSubsidy = nSubsidy * 20 ;  //1000% until block 35k
            }

            else if(nBestHeight <= 45000)
            {
            nSubsidy = nSubsidy * 10 ;   //500% until block 45k
            }

            else if(nBestHeight >= 100000)
            {
            nSubsidy = nSubsidy * 2 ;   //100% until block 100k
            }

            else if(nBestHeight >= 135000)
            {
            nSubsidy = nSubsidy * 10 ;   //500% until block 135k
            }
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nCoinAge);

    return nSubsidy * 1 ;   //50% Final
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VestxCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "What happens on January 19, 2038? ";
        CTransaction txNew;
        txNew.nTime = 1530852488;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1530852488 ;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 7083462 : 7083462;
        ...
        assert(block.hashMerkleRoot == uint256("0xd4153d29499efb58b456a2db7c71f8ba4154b35411c022602d6256c89c84ede8"));
...
unsigned char pchMessageStart[4] = { 0x77, 0xf9, 0x3A, 0x3e };
